...
export function BookingCard({ booking, isInterviewer, onStatusUpdate }) {
  ...
  const canSubmit = booking.status === 'Confirmed'; // upcoming and past
        ...
        <div className="flex shrink-0 flex-col items-start gap-2 sm:items-end">
          ...
          {(isInterviewer ? booking.candidateId : booking.interviewerId) && (
            <Link
              ...
              className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-600 transition hover:border-brand-300 hover:bg-brand-50 hover:text-brand-700 focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-1"
            >
              <svg className="h-3.5 w-3.5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24" aria-hidden="true">
